...
hstock_detailed <- function(folder, years){
  ...
  hh <- purrr::map_dfr(years, function(year){
    ...
    if(file.exists(paste(folder, stringr::str_subset(stringr::str_subset(files, pattern = key$ukda), pattern = key$firstimp_int), sep="")) & length(stringr::str_subset(stringr::str_subset(files, pattern = key$ukda), pattern = key$firstime_int))>0){firstimp_int <- haven::read_spss(paste(folder, stringr::str_subset(stringr::str_subset(files, pattern = key$ukda), pattern = key$firstimp_int), sep=""))}
    ...
    if(file.exists(paste(folder, stringr::str_subset(stringr::str_subset(files, pattern = key$ukda), pattern = key$shared), sep="")) & length(stringr::str_subset(stringr::str_subset(files, pattern = key$ukda), pattern = key$shared))>0){shared <- haven::read_spss(paste(folder, stringr::str_subset(stringr::str_subset(files, pattern = key$ukda), pattern = key$shared), sep=""))}
